...
public class Renderer {
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		colourToReturn = C_diff.scale(colourToReturn.add(I_a));
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			//ColorRGB colDif = C_diff.scale(I.scale(k_d * Math.max(0, N.normalised().dot(L))));
			//ColorRGB colSpec = C_spec.scale(I.scale(k_s * Math.pow(Math.max(0, R.dot(V)), alpha)));
			ColorRGB colDif = C_diff.scale(k_d).scale(I).scale(Math.max(0, N.normalised().dot(L)));
			ColorRGB colSpec = C_spec.scale(k_s).scale(I).scale(Math.pow(Math.max(0, R.dot(V)), alpha));

			//Check for shadowRays
			Ray shadowRay = new Ray(P.add(N.scale(EPSILON)), L);
			RaycastHit shadowHit = scene.findClosestIntersection(shadowRay);
			if (shadowHit.getDistance() > distanceToLight){
				colourToReturn = colourToReturn.add(colDif.add(colSpec));
			}
			...
			//Amb only
			//Amb + diff
			//colourToReturn = colourToReturn.add(colDif);
